...
SELECT from_c.num, from_c.company, from_c.name, to_l.num, to_l.company
FROM (SELECT DISTINCT stopd.name, c.company, c.num
  FROM route c
  ...
  JOIN (SELECT *
  FROM (SELECT DISTINCT stopb.name, a.company, a.num
    FROM route a
      JOIN route b ON (a.company=b.company AND a.num=b.num)
      JOIN stops stopa ON (a.stop=stopa.id)
      JOIN stops stopb ON (b.stop=stopb.id)
